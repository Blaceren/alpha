...
test("checkpoint desktop", async ({ page }) => {
  await open(page, "/path?scenario=checkpoint", DESKTOP);
  await grab(page, DESKTOP, "path-checkpoint-desktop-1440x900.png");
});
...
test("module navigator focus (desktop)", async ({ page }) => {
  await open(page, "/path?scenario=active", DESKTOP);
...
test("advanced L85 desktop", async ({ page }) => {
  await open(page, "/path?scenario=advanced", DESKTOP);
  await grab(page, DESKTOP, "path-advanced-level-85-desktop.png");
});
