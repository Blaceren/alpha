/**
 * A test double for `window.turnstile` (AFD-3A3).
 *
 * The real widget is a cross-origin iframe served by Cloudflare, so jsdom can
 * never render it. What the CRM actually owns is the CONTRACT around it:
 ...
 *
 * Ported from the Academy's identical double. The two frontends share no module
 * boundary; duplicating 100 lines of test scaffolding is cheaper to keep honest
 * than a dependency between two independently deployed applications.
 */
...
export function installTurnstileDouble(options: InstallOptions = {}): TurnstileDouble {
  ...
  window.turnstile = {
    render(_container: HTMLElement, opts: TurnstileRenderOptions) {
      ...
      byId.set(id, opts);
      if (autoSolve) {
        // Solved SYNCHRONOUSLY, inside the render call.
        //
        // The Academy's copy of this double defers with `queueMicrotask` so the
        // token never exists during the render pass. That is more faithful, and
        // it does not work under this package's React/RTL versions: a deferred
        // callback drives its own `act()`, which lands in the middle of the NEXT
        // test's `render()` and produces "overlapping act() calls" — after which
        // React discards the tree and every subsequent query finds an empty
        // body. Solving here is safe because `turnstile.render` is itself called
        // from inside a React effect, which RTL has already wrapped in `act`.
        //
        // The cases that care about the unsolved state install the double with
        // `autoSolve: false` and drive `solve()` explicitly, so nothing is lost.
